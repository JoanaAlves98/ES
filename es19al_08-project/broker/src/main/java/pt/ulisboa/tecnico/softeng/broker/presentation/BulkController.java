...
public class BulkController {
	...
	@RequestMapping(value = "{number}/cancel", method = RequestMethod.POST)
	    public String cancelBooking(final Model model, @PathVariable final String brokerCode, @PathVariable final String number,@PathVariable final String reference) {
            logger.debug("cancelBooking");

	        final HotelInterface hotelInterface = new HotelInterface();
	
	        final String cancelReservation = hotelInterface.cancelBooking(reference);

            return "redirect:/brokers/"  + brokerCode + "/bulks/" + number + "/references/" ;
